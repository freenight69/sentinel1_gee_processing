...
def lin_to_db2_hhhv(image):
    ...
    """
    db = ee.Image.constant(10).multiply(image.select(['HH', 'HV']).log10()).rename(['HH', 'HV'])
    return image.addBands(db, None, True)
...
def add_ratio_lin_hhhv(image):
    ...
    """
    ratio = image.addBands(image.select('HH').divide(image.select('HV')).rename('HHHV_ratio'))
